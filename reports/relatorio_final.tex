...
%----------------------------------------------------------------------------------------
\usepackage{lmodern}      % Torna a fonte padrão (CM) vetorial e escalável
\usepackage{anyfontsize}  % Permite tamanhos de fonte arbitrários sem avisos
...
\usepackage[
    backend=biber,
    style=abnt,          % Estilo de formatação ABNT
    citestyle=authoryear, % Citação tipo (AUTOR, Ano) no texto
    sorting=nty,          % Ordenar por Nome, Título, Ano
    url=true,             % Mostrar URLs
    doi=true              % Mostrar DOIs se houver
]{biblatex}

% Carrega o arquivo .bib que criei acima
\addbibresource{referencias.bib}

%----------------------------------------------------------------------------------------
% LAYOUT SETTINGS (CORRIGIDO: MARGENS SIMÉTRICAS)
%----------------------------------------------------------------------------------------
% Alterei right=2cm para right=2.5cm para igualar as margens e evitar o "pulo" no sumário
\usepackage[a4paper,
...
% splitdot — versão corrigida para alinhamento perfeito
\newcommand*\splitdot{%
  \hspace{0.3em}%
  \raisebox{0.4ex}{%
    \tikz{%
...
% BACKGROUND GRAPHICS (MANTIDO IGUAL)
\begin{tikzpicture}[remember picture, overlay]
...
\begin{center}
    ...
    % --- HIERARQUIA (COM QUEBRA DE LINHA SOLICITADA) ---
    {\color{white}\bfseries\textls[100]{CENTRO DE TECNOLOGIA \\ DEPARTAMENTO DE ENGENHARIA ELÉTRICA}}\\[0.8cm]

    % --- MATÉRIA (INSERIDA CONFORME PEDIDO) ---
    {\color{white}\bfseries\textls[50]{
        ELE 604 – \textcolor{white}{\MakeUppercase{Redes Neurais Artificiais}}
    }}
    ...
    \lettrine[lines=2, lraise=0.1]{A}{NÁLISE DE GENERALIZAÇÃO} \\ 
    \textls[50]{\MakeUppercase{em Redes Neurais}}}
    
    \vfill
    
    % --- SUBTÍTULO ---
    \begin{futuristicbox}
    \textls[80]{REGRESSÃO DE PREÇOS IMOBILIÁRIOS COM VALIDAÇÃO CRUZADA {\color{accent}\textbf{K-FOLD}}}
    \end{futuristicbox}
    ...
    % --- PROFESSOR ---
    {\color{white}\small\bfseries\textls[50]{\MakeUppercase{Professor: \\ Prof. Dr. ALLAN DE MEDEIROS MARTINS}}}
    
    \vfill
    
    % --- DATA ---
    {\color{white}\large\bfseries\textls[100]{\MakeUppercase{Natal-RN} \splitdot 2025}}
...
\lettrine[lines=3, lhang=0.1, loversize=0.2]{\color{accent}\textbf{E}}{ste relatório} apresenta uma análise da capacidade de generalização de redes neurais do tipo Multi-Layer Perceptron (MLP) aplicada à regressão de preços imobiliários, utilizando o dataset Boston Housing. O objetivo central foi mitigar o \textit{overfitting} inerente a conjuntos de dados pequenos e garantir uma avaliação de desempenho robusta e confiável.

A metodologia adotou a Validação Cruzada K-Fold ($K=5$) e um pipeline rigoroso para prevenção de \textit{Data Leakage} durante a normalização. Foram implementadas técnicas de regularização, incluindo Dropout e Weight Decay (L2), combinadas com Otimização Bayesiana (via Optuna) para o ajuste fino de hiperparâmetros.

Os resultados demonstraram que a aplicação de regularização dinâmica reduziu drasticamente o gap entre as curvas de treino e validação. O modelo otimizado alcançou um Erro Quadrático Médio (MSE) de 13.02 e um coeficiente de determinação ($R^2$) de 0.857, evidenciando que a combinação de práticas de MLOps e otimização automatizada é essencial para a construção de preditores robustos em cenários de \textit{Small Data}.
...
\begin{tikzpicture}
    ...
    \begin{minipage}{0.8\textwidth}
        \centering
        \textbf{Palavras-chave:} Redes Neurais $\cdot$ Regressão $\cdot$ Boston Housing $\cdot$ K-Fold $\cdot$ Optuna $\cdot$ Generalização
    \end{minipage}
    };
\end{tikzpicture}
\end{center}
...
\lettrine[lines=3, lhang=0.1, loversize=0.2]{\color{accent}\textbf{T}}{his report} presents an analysis of the generalization capabilities of Multi-Layer Perceptron (MLP) neural networks applied to real estate price regression using the Boston Housing dataset. The main objective was to mitigate overfitting, which is typical in small datasets, and to ensure a robust and reliable performance evaluation.
...
\begin{tikzpicture}
    ...
    \begin{minipage}{0.8\textwidth}
        \centering
        \textbf{Keywords:} Neural Networks $\cdot$ Regression $\cdot$ Boston Housing $\cdot$ K-Fold $\cdot$ Optuna $\cdot$ Generalization
    \end{minipage}
    };
\end{tikzpicture}
\end{center}
...
A predição de preços imobiliários é um problema clássico de \textbf{regressão} em Machine Learning \parencite{bishop2006pattern, james2013statistical, kelleher2015fundamentals}, com aplicações diretas em planejamento urbano, avaliação de investimentos e políticas públicas. Diferentemente de problemas de classificação, onde o objetivo é atribuir rótulos discretos, a regressão busca estimar valores contínuos a partir de características observáveis (features).
...
O conceito central deste trabalho é a \textbf{generalização}: a capacidade de um modelo de aprendizado de máquina desempenhar bem em dados \textit{não vistos} durante o treinamento. A generalização está intrinsecamente ligada ao trade-off entre \textbf{viés (bias)} e \textbf{variância (variance)}, conforme discutido extensivamente por \textcite{hastie2009elements}:

\begin{highlightbox}[title={Conceitos Fundamentais}]
\begin{itemize}
    \item \textbf{Viés Alto (Underfitting):} O modelo é excessivamente simples e não consegue capturar os padrões complexos presentes nos dados. Resulta em alto erro tanto no conjunto de treino quanto no de validação.
    \item \textbf{Variância Alta (Overfitting):} O modelo é excessivamente complexo e ``memoriza'' os dados de treino, incluindo ruídos e outliers. Apresenta baixo erro no treino, mas alto erro na validação.
...
\end{equation}
onde $y$ é o valor real, $\hat{f}(x)$ é a predição do modelo, e $\sigma^2$ é o ruído irredutível \parencite{hastie2009elements}.
...
\begin{itemize}
    ...
    \item Utilizar Otimização Bayesiana \parencite{optuna2019} para encontrar hiperparâmetros ótimos.
...
O dataset foi obtido da fonte original \url{http://lib.stat.cmu.edu/datasets/boston}, compilado e analisado originalmente por \textcite{harrison1978hedonic} e posteriormente revisado sob a ótica de diagnósticos de regressão por \textcite{belsley1980regression}. As 13 features são:
...
A normalização é crítica para redes neurais treinadas via Gradiente Descendente, conforme estabelecido nos trabalhos seminais de \textcite{lecun1998efficient}, pois:
...
Implementamos uma rede neural totalmente conectada (feedforward) utilizando o framework PyTorch \parencite{paszke2019pytorch}, com a seguinte arquitetura:
...
A ReLU resolve o problema de desaparecimento de gradiente e acelera o treinamento, propriedades fundamentais demonstradas por \textcite{nair2010relu} e analisadas por \textcite{glorot2011deep}.
...
Implementamos \textbf{Dropout} com taxa de $p=0.3$ (30\%) após cada camada oculta. Durante o treinamento, neurônios são "desligados" aleatoriamente. Segundo \textcite{srivastava2014dropout}:
...
Isso previne pesos excessivamente grandes e suaviza as superfícies de decisão \parencite{bishop2006pattern}.
...
Utilizamos o \textbf{Adam} (Adaptive Moment Estimation), proposto por \textcite{kingma2015adam}, com taxa de aprendizado $\alpha = 0.001$:
...
O Early Stopping monitora o erro de validação e interrompe o treinamento se não houver melhoria por $P$ épocas consecutivas (patience). Isso previne overfitting ao parar antes que o modelo comece a "memorizar" o conjunto de treino \parencite{goodfellow2016deep}.
...
Para alcançar o estado da arte, aplicamos \textbf{Otimização Bayesiana} usando a biblioteca Optuna \parencite{optuna2019}. Esta técnica supera o Grid Search tradicional e o Random Search \parencite{bergstra2012random} ao aprender com resultados de trials anteriores.
...
O \textbf{HyperbandPruner} interrompe trials não promissores precocemente, economizando até 70\% do tempo computacional \parencite{optuna2019}. Se após 10 épocas um trial apresenta MSE muito superior ao melhor resultado atual, ele é podado (\textit{pruned}).
...
Classificamos o desempenho final como \textbf{Boa Generalização} \parencite{hastie2009elements}, baseado em três critérios:
...
\begin{enumerate}
    \item \textbf{Dropout (30\%):} Reduziu drasticamente o overfitting inicial. O gap treino-validação caiu de 181\% para aproximadamente 30-40\%, demonstrando a eficácia da regularização estocástica \parencite{srivastava2014dropout}.
...
\begin{itemize}
    ...
    \item \textbf{Trade-off Bias-Variance:} O modelo otimizado apresentou MSE médio menor, mas desvio padrão maior (4.62 vs 2.47), indicando maior variância. Para aplicações que priorizam estabilidade, o modelo base pode ser preferível.
\end{itemize}
...
\tikzset{
    complexNode/.style={
        draw=none, text=white, font=\bfseries\Large, align=center,
        blur shadow={shadow blur steps=5, shadow xshift=2pt, shadow yshift=-2pt},
        inner sep=8pt 
    },
    terminal/.style={
        complexNode, rectangle, rounded corners=12pt,
        top color=accentTeal!80!black, bottom color=accentTeal,
        minimum width=5.5cm, minimum height=1.6cm
    },
    process/.style={
        complexNode, rectangle, rounded corners=4pt,
        top color=accentBlue!80!black, bottom color=accentBlue,
        minimum width=5.5cm, minimum height=1.5cm
    },
    decision/.style={
        complexNode, diamond, aspect=1.8,
        top color=accentOrange!80!black, bottom color=accentOrange,
        minimum width=5.0cm, font=\large\bfseries\itshape 
    },
    loopNode/.style={
        complexNode, regular polygon, regular polygon sides=6, shape border rotate=90,
        top color=accentPurple!80!black, bottom color=accentPurple,
        inner sep=-2pt, minimum width=3.5cm, font=\large\bfseries 
    },
    flowLine/.style={
        draw=primaryDark!80, line width=2.0pt, rounded corners=8pt, ->, >=LaTeX
    },
    feedbackLine/.style={
        flowLine, dashed, draw=alertRed, line width=1.5pt
    },
    clusterBox/.style={
        draw=primaryDark!40, dashed, line width=1.5pt, fill=bgDeep,
        inner sep=20pt, rounded corners=20pt, drop shadow={opacity=0.15}
    }
}

\resizebox{\textwidth}{!}{%
% Aumentei a distância horizontal (2.5cm) para as linhas verticais terem espaço
\begin{tikzpicture}[node distance=1.8cm and 2.5cm]

    % === FLUXO 1: ESQUERDA ===
    \node (load) [terminal] {Carregar Dataset};
    \node (kfold) [loopNode, below=of load] {\shortstack{Iterador\\K-Fold}};
    \node (split) [process, below=of kfold] {Split Treino/Val};
    \node (scaler) [process, below=of split] {Fit \& Transform};
    
    % === FLUXO 2: CENTRO ===
    \node (dataloader) [process, right=of kfold] {DataLoaders};
    \node (initmlp) [process, below=of dataloader] {Inicializar Pesos};
    \node (trainloop) [loopNode, below=of initmlp] {Training Loop};
    \node (step) [process, below=of trainloop] {Backpropagation};
    \node (valmetric) [process, below=of step] {Métricas Validação};

    % === FLUXO 3: DIREITA ===
    % Posicionando Early Stop alinhado com o Training Loop para simetria visual
    \node (earlystop) [decision, right=of trainloop] {Early Stop?};
    \node (checkpoint) [process, above=of earlystop, top color=primaryDark!80, bottom color=primaryDark] {Salvar Checkpoint};
    % Restaurar Melhor fica abaixo da decisão
    \node (loadbest) [process, below=of earlystop, yshift=-1.5cm] {Restaurar Melhor}; 
    \node (evaluate) [process, below=of loadbest] {Avaliar Fold $k$};
    \node (stats) [terminal, below=of evaluate] {Estatísticas Finais};

    % === CLUSTERS (FUNDO) ===
    % Agora renderiza corretamente porque a layer 'background' foi ativada
    \begin{pgfonlayer}{background}
        \node [clusterBox, fit=(load) (scaler), label={[anchor=south east, font=\itshape\bfseries\Large, text=gray]north east:PRE-PROCESSAMENTO}] (grp1) {};
        \node [clusterBox, fit=(dataloader) (valmetric), label={[anchor=south east, font=\itshape\bfseries\Large, text=gray]north east:KERNEL DE TREINO}] (grp2) {};
        \node [clusterBox, fit=(earlystop) (stats) (checkpoint), label={[anchor=south east, font=\itshape\bfseries\Large, text=gray]north east:SUPERVISÃO}] (grp3) {};
    \end{pgfonlayer}

    % === LINHAS (DESENHADAS NO MEIO) ===
    \begin{pgfonlayer}{lines}
        
        % Coluna 1
        \path [flowLine] (load) edge (kfold) (kfold) edge (split) (split) edge (scaler);
        
        % Transição 1 -> 2 (CORREÇÃO DE "SETA SUBINDO")
        % Sai pela direita (east) em vez de baixo, evitando passar na frente da caixa
        \draw [flowLine] (scaler.east) -- ++(1.2,0) |- (dataloader.west);

        % Coluna 2
        \path [flowLine] (dataloader) edge (initmlp) (initmlp) edge (trainloop) (trainloop) edge (step) (step) edge (valmetric);

        % Transição 2 -> 3
        % Sai de Validação, vai para direita, sobe e entra na esquerda do Early Stop
        \draw [flowLine] (valmetric.east) -- ++(0.5,0) |- (earlystop.west);

        % === LÓGICA EARLY STOP (SEM CRUZAMENTOS) ===
        
        % NÃO (Melhorou): Sai por CIMA -> Checkpoint -> Volta pro loop
        \draw [feedbackLine] (earlystop.north) -- node[right, font=\normalsize\bfseries, text=alertRed, xshift=2pt] {Não} (checkpoint.south);
        
        % Do checkpoint volta para o training loop por cima
        \draw [feedbackLine] (checkpoint.west) -| ($(trainloop.north)+(0,0.5)$) -- (trainloop.north);

        % SIM (Piorou): Sai por BAIXO -> Restaurar Melhor
        \draw [flowLine] (earlystop.south) -- node[right, font=\normalsize\bfseries, yshift=-2pt] {Sim (Stop)} (loadbest.north);

        % Coluna 3 Final
        \path [flowLine] (loadbest) edge (evaluate) (evaluate) edge (stats);

        % Loop Principal (K-Fold)
        % Afastado bem para a esquerda para não cortar o "Pre-processamento"
        \draw [flowLine, color=primaryDark] (stats.south) -- ++(0,-0.8) -| ($(grp1.west)+(-1.0,0)$) |- (kfold.west);

    \end{pgfonlayer}

\end{tikzpicture}
}
\caption{Fluxograma Neural K-Fold Ajustado.}
